...
class Converter:
    ...
    def handle_cmd(self, cmd, args):
        if cmd == "!chapter":
            ...
            args = args.replace("/", "+")
            self.current_chapter = space_to_dash(args.strip())
            self.current_section = ""
            self.switch_file("README.md", self.current_chapter)
            self.current_listing = 0
            self.curof.write("= {} =".format(self.current_chapter))
        elif cmd == "!section":
            self.info("processing section", args, "in", self.current_chapter)
            args = args.replace("/", "+")
            self.current_section = space_to_dash(args.strip())
            self.switch_file(self.current_section, self.current_chapter)
            self.curof.write("== {} ==".format(self.current_section))
        elif cmd == "!subsection":
            self.curof.write("=== {} ===".format(args))
        else:
